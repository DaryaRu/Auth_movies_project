...
class RolesAbstractRepository(ABC):
    @abstractmethod
    async def create_role(self, name: str, description: str | None, is_active: bool) -> RoleORM:
...
class RolesPostgreSQLRepository(RolesAbstractRepository, BasePostgreSQLRepository):
    ...
    async def create_role(self, name: str, description: str | None = None, is_active: bool = True) -> RoleORM:
        return await self.add_one(name=name, description=description, is_active=is_active)
    ...
    async def assign_permission_to_role(self, role_id: UUID, permission_id: UUID) -> None:
        query = insert(role_permissions_table).values(role_id=role_id, permission_id=permission_id)
        try:
            await self._session.execute(query)
        except IntegrityError as ex:
            if isinstance(ex.orig.__cause__, UniqueViolationError):
    ...
    async def get_user_permissions(self, user_id: UUID) -> list[PermissionORM]:
        query = (
            ...
            .join(role_permissions_table, PermissionORM.id == role_permissions_table.c.permission_id)
