"""
Benchmark-скрипт для тестирования производительности ClickHouse.
...
DATA_DIR = os.path.join(_SCRIPT_DIR, "data", "ch_data")
load_dotenv(ENV_PATH)


def start_docker_container() -> None:
    """Запускает контейнер через docker compose. Использует локальный образ если есть."""
    print("\n[START] Поднимаем контейнер ClickHouse...")

    # Проверяем, есть ли локальный образ
    try:
        result = subprocess.run(
            ["docker", "images", "-q", "clickhouse-server:amd64"],
            capture_output=True,
            text=True,
            check=True
        )
        if result.stdout.strip():
            print("  Локальный образ clickhouse-server:amd64 найден - используем его")
        else:
    ...
    time.sleep(5)  # Ждем пока ClickHouse полностью запустится


def stop_docker_container() -> None:
    """Останавливает и удаляет контейнер через docker compose."""
    print("\n[STOP] Останавливаем контейнер ClickHouse...")
    subprocess.run(
...
class Config:
    ...
    CH_PORT: int = int(os.environ.get("CH_PORT", "8123"))
    CH_USER: str = os.environ.get("CH_USER", "default")
    CH_PASSWORD: str = os.environ.get("CH_PASSWORD", "123")
    CH_DATABASE: str = os.environ.get("CH_DATABASE", "analytics")
...
# ---------------------------------------------------------------------------
def create_client(database: Optional[str] = None) -> Client:
...
# Подготовка/schema
# ---------------------------------------------------------------------------
def setup_database(client: clickhouse_connect.client.Client) -> None:
    """Создаёт БД и таблицу для тестов."""
    client.command(f"CREATE DATABASE IF NOT EXISTS {config.CH_DATABASE}")
    print(f"  База {config.CH_DATABASE} готова.")

    db_client = create_client(database=config.CH_DATABASE)
    db_client.command("DROP TABLE IF EXISTS analytics.events")
    db_client.command(
        '''
...
def writer_worker() -> None:
    ...
    client = create_client(database=config.CH_DATABASE)
    batch_data = generate_batch(config.BATCH_SIZE)
    client.insert(
        "analytics.events",
        batch_data,
        column_names=["user_id", "event_type", "object_id", "payload", "event_time"],
    )
    client.close()
...
ANALYTIC_QUERIES: list[tuple[str, str]] = [
    (
        ...
        '''
         SELECT
             JSONExtractString(payload, 'genre') AS genre,
             count() AS cnt
         FROM analytics.events
         WHERE event_type = 'film_search'
          AND JSONExtractString(payload, 'genre') = 'action'
        GROUP BY genre
        ORDER BY cnt DESC
        ''',
    ),
...
def run_single_query(
    client: clickhouse_connect.client.Client,
    name: str,
    sql: str,
    runs: int,
) -> QueryResult:
    ...
    for i in range(runs):
        ...
        result.rows_returned = len(query_result.result_rows)

        if hasattr(query_result, "query_id") and query_result.query_id:
            try:
                stats = client.command(
                    f"SELECT rows_read, bytes_read "
                    f"FROM system.query_log "
                    f"WHERE query_id = '{query_result.query_id}' "
                    f"LIMIT 1"
                )
                if stats:
                    row = stats[0]
                    result.rows_read = int(row[0])
                    result.bytes_read = int(row[1])
            except Exception:
                pass
...
def run_read_tests(label: str = "БЕЗ НАГРУЗКИ") -> List[QueryResult]:
    ...
    for name, sql in ANALYTIC_QUERIES:
        qr = run_single_query(client, name, sql, config.QUERY_RUNS)
        results.append(qr)
...
def generate_report(
    ...
    w(f"  Целевой хост (ВМ):         {config.CH_HOST}:{config.CH_PORT}")
    w(f"  База данных:               {config.CH_DATABASE}")
    ...
    if all_pass_no_load and all_pass_concurrent:
        ...
        w("  ClickHouse удовлетворяет требованию по скорости")
...
def run_benchmark() -> None:
    ...
    print(f"  Хост: {config.CH_HOST}:{config.CH_PORT}")
    print(f"  БД:   {config.CH_DATABASE}")
    print("=" * 60)

    # 1. Setup
    print("\n[1/4] Подготовка базы данных...")
    main_client = create_client()
    setup_database(main_client)
    main_client.close()
    ...
    report_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "CH_BENCHMARK_REPORT.txt")
...
def main() -> None:
    ...
    try:
        ...
        # Останавливаем контейнер гарантированно
