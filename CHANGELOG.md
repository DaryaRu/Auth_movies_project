...
- `src/schemas/oauth_accounts.py` — схема ответа `OAuthAccountResponseScheme`.
...
- `src/services/oauth.py` — сервис `OAuthService`: формирует URL редиректа, обменивает code на данные пользователя через Authlib, находит или создаёт пользователя, выдаёт JWT-токены.
...
- `src/api/v1/oauth.py` — два эндпоинта:
  - `GET /api/v1/auth/{provider}/` — генерирует `state`, сохраняет его в httpOnly cookie `oauth_state` (TTL 5 минут), возвращает 302-редирект на страницу авторизации провайдера.
