...
LOCAL_DATETIME_REGEX = re.compile(r"^\d{4}-\d{2}-\d{2} \d{2}:\d{2}$")
...
class NotificationTemplateForm(forms.Form):
    ...
    code = forms.CharField(
        ...
        help_text=_(
            "Уникальный код шаблона (например: review_liked). Изменение кода невозможно."
        ),
    )
    ...
    subject = forms.CharField(
        ...
        required=False,
        widget=forms.TextInput(
            attrs={"class": "vTextField", "maxlength": 500}
        ),
        help_text=_(
            "Тема сообщения. Поддерживает переменные в виде {{movie_title}}."
        ),
    )
    body = forms.CharField(
        ...
        help_text=_(
            "Содержимое сообщения. Поддерживает переменные в виде {{movie_title}}."
        ),
    )
    allowed_variables = forms.CharField(
        ...
        help_text=_(
            "Список разрешённых переменных через запятую (например: user_name, movie_title)"
        ),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if api_data and not any(k in kwargs for k in ("data", "files")):
            ...
            if isinstance(allowed_vars, list):
                ...
                initial_data["allowed_variables"] = (
                    str(allowed_vars) if allowed_vars else ""
                )
        ...
        if self._api_data and self._api_data.get("code"):
            obj, _ = NotificationTemplate.objects.get_or_create(
                ...
                defaults={
                    ...
                    "allowed_variables": self._api_data.get(
                        "allowed_variables", []
                    ),
    ...
    def save_template(
        self, template_id: str | None = None, auth_token: str | None = None
    ) -> dict:
        ...
        if template_id:
            try:
                return api_client.update_template(
                    template_id, data, auth_token
                )
            except TemplateNotFoundError as exc:
                raise forms.ValidationError(
                    _("Template not found in service")
                ) from exc
            ...
            try:
                ...
                raise forms.ValidationError(
                    _("Template with this code already exists")
                ) from exc
            except APIError as e:
                raise forms.ValidationError(str(e)) from e
...
class AdminMailingForm(JSONPayloadMixin, forms.Form):
    ...
    subscription_level = forms.IntegerField(
        ...
        help_text=_(
            'Минимальный уровень подписки (для фильтра "По уровню подписки")'
        ),
    )
    payload_json = forms.CharField(
        ...
        required=False,
        help_text=_(
            "Дополнительные данные для подстановки в шаблон (JSON). "
    ...
    send_mode = forms.ChoiceField(
        ...
        choices=[
            ...
            ("scheduled", _("Schedule for recipient local time")),
        ],
        widget=forms.RadioSelect(),
        initial="now",
    )
    scheduled_local_datetime = forms.CharField(
        label=_("Scheduled at (recipient local time)"),
        required=False,
        max_length=16,
        widget=forms.TextInput(
            attrs={
        ...
        help_text=_(
            "Дата и время по таймзоне получателя, формат ГГГГ-ММ-ДД ЧЧ:ММ "
            "(например 2057-09-18 10:00)."
        ),
    )

    def clean_scheduled_local_datetime(self):
        """Проверить формат ГГГГ-ММ-ДД ЧЧ:ММ и распарсить в datetime."""
        value = self.cleaned_data.get("scheduled_local_datetime", "").strip()
        if not value:
            return None
        if not LOCAL_DATETIME_REGEX.match(value):
            raise forms.ValidationError(
                _(
                    "Некорректный формат даты и времени (ожидается ГГГГ-ММ-ДД ЧЧ:ММ)"
                )
            )
        try:
            return datetime.strptime(value, "%Y-%m-%d %H:%M")
        except ValueError as e:
            raise forms.ValidationError(_("Некорректная дата")) from e

    def clean(self):
        """subscription_level обязателен при audience_type=subscription;
        scheduled_local_datetime обязателен при send_mode=scheduled."""
        cleaned = super().clean()
        if cleaned.get("audience_type") == "subscription" and not cleaned.get(
            "subscription_level"
        ):
            self.add_error(
                "subscription_level",
                _('Required when audience type is "subscription"'),
            )

        if (
            cleaned.get("send_mode") == "scheduled"
            and not cleaned.get("scheduled_local_datetime")
            and "scheduled_local_datetime" not in self.errors
        ):
            self.add_error(
                "scheduled_local_datetime",
                _('Required when send is "Schedule for recipient local time"'),
            )
        return cleaned
    ...
    def build_api_data(self, created_by: str) -> dict:
        ...
        if self.cleaned_data.get("send_mode") == "scheduled":
            dt = self.cleaned_data.get("scheduled_local_datetime")
            scheduled_local_datetime = (
                dt.strftime("%Y-%m-%dT%H:%M:%S") if dt else None
            )
    ...
    def save_mailing(
        self, auth_token: str | None = None, created_by: str | None = None
    ) -> list[dict]:
