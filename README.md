...
- **notifications-service** — рассылка уведомлений (email, push) по шаблонам: персональные события, авто-триггеры по изменению контента, ручные рассылки из админки. Отдельные воркер и шедулер — API и шедулер публикуют события в Kafka, воркер их читает и рассылает.
- **ws-gateway** — WebSocket-шлюз для push-уведомлений. Держит соединения с клиентами (аутентификация JWT), слушает Redis Pub/Sub и доставляет тем, кто сейчас подключён.
...
Сервис рассылки уведомлений по email и push, с рендером текста из шаблонов (Jinja2-плейсхолдеры).
...
- **Push через WebSocket** — шаблон с `channel=push`, доставка через `ws-gateway`: воркер публикует в Redis Pub/Sub (`push:{user_id}`), `ws-gateway` доставляет тем, кто сейчас подключён по WebSocket. Без гарантии доставки — если пользователь офлайн, уведомление теряется (в отличие от email)
