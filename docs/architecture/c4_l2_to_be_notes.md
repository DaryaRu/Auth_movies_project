...
- **aiokafka** / **FastStream** — producer в analytics-service на aiokafka, consumer в analytics-etl через FastStream + KafkaBroker.
- **aiochclient** — async-клиент ClickHouse используется в analytics-etl.

## Kafka: топики и партиции

**Топики:** `user-activity` (основной), `user-activity.dlq` (dead-letter — сообщения, не прошедшие валидацию схемы событий в analytics-etl)

**Producers** (пишут в Kafka):
- `analytics-service` — получает событие от пользователя и публикует в топик `user-activity`
- `analytics-etl` — публикует в `user-activity.dlq` невалидные сообщения

**Consumers** (читают из Kafka):
- `analytics-etl` — подписывается на топик `user-activity` в consumer group `analytics-etl` (настройка через `KAFKA_GROUP_ID`), читает события и пишет в ClickHouse
...
- `player_action`: `{"action": "seek", "position_sec": 90}` (`action` — `play`, `pause` или `seek`; `position_sec` заполняется только для `seek`)
...
- `film_start` — начало просмотра
- `film_progress` — прогресс просмотра (позиция и длительность)
- `video_quality_changed` — смена качества видео
- `video_completed` — просмотр до конца
- `player_action` — действие в плеере (play/pause/seek)
